...
function detect_e()
    ...
    for k,v in pairs(players) do
        ...
        local dot_x = sensor_center["x"] - (v["x"]-entdetector_coords["x"])*2
        local dot_y = sensor_center["y"] - (v["z"]-entdetector_coords["z"])*2
        local label_x = dot_x*2 - string.len(ent_name) * 2
        local label_y = dot_y*2 - 10

        if  ent_dot[ent_name] == nil then
            ent_dot[ent_name] = {}
            ent_dot[ent_name]["dot"] = ghelper.dot(dot_x, dot_y, 2, green_color)
            ent_dot[ent_name]["label"] = ghelper.infoText(ent_name, label_x, label_y, 0.5, green_color)
        else
            if math.abs(v["range"])<9.5 then
                ent_dot[ent_name]["dot"].setAlpha(1)
                ent_dot[ent_name]["label"].setAlpha(1)
                ent_dot[ent_name]["dot"].setPosition(dot_x, dot_y)
                ent_dot[ent_name]["label"].setPosition(label_x, label_y)
            else
                ent_dot[ent_name]["dot"].setAlpha(0.1)
                ent_dot[ent_name]["label"].setAlpha(0.1)
            end
        end
    end
end

local detect_timer = event.timer(0.2, detect_e, math.huge)

function cleanExit()
    event.cancel(detect_timer)
    running = false
    print("exiting")
end
...
local tab_functions = {
    ...
    [3] = function() cleanExit() os.execute("sensor_hud.lua") end,
...
function myEventHandlers.motion(_, x, y, z, entity_name)
    if detect_motion == true then
        local dot_x = sensor_center["x"] + x*2
        local dot_y = sensor_center["y"] - z*2
...
ghelper.dot(sensor_center["x"], sensor_center["y"], 3, primary_color)
