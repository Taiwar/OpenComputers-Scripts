...
function updatePowerDisplay(energy, capacity, y)
    ...
    local energy_width = energy_ratio*100*((base_width - 30)/100)
...
function calculateNetEnergy(curr_energy)
    ...
    net_energy_info.setText("Net-Energy in kRF/t: "..roundTo(energy_dif/1000, 2)/20)
...
ghelper.bgBox(base_x - 4, base_y - 10, 60, base_width, primary_color_dark)
ghelper.headlineText("Draconic Energy Sphere", base_x, base_y, base_width, base_text_scale, primary_color)
...
while true do
    ...
    if port == 8000 then
        power_info.setText("Energy stored in MRF: "..roundTo(msg[1]/1000000, 2))
